...
request through a relay, and gives you back the exact `httpx.Response` you'd get without
...
- **Three modes, one class.** Route through a **Cloudflare Worker relay** (`server`, plus
  `client_id` + `client_secret` when it's protected by Cloudflare Access), a **standard
  HTTP/HTTPS proxy** (`proxy`), or go **direct** (no arguments) — behaving identically to
  plain httpx.
...
# Route through a Cloudflare Access–protected Worker relay
client = GetAround(
    server="https://relay-server",
...
The Cloudflare relay mode talks to its companion Worker,
...
the `GetAround` relay **and** directly through plain httpx, then the two JSON responses are
...
Building a Cloudflare-relay client from stored credentials? Install the `testing` extra for
